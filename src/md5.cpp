...
char *MD5::hex_digest(){
  ...
  int i;
  //char *s= new char[33];
  char *s = hex_digestoutput; // mod by ei, previous line was memory leak!!!
  s[0] = '\0';
...
//simple usage of this module
std::string calculate_md5(const std::string& str){
	MD5 context;
	context.update((unsigned char*)str.c_str(), (unsigned int)str.length());
	context.finalize();
